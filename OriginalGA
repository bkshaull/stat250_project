...
clear
n=50; %number trials array=(n,N,t)
...
solution=1; %if solution is 1, run genetic algorithm plugging in the solution, if zero, don't run algorithm on solution
pause on 

%Generating First Generation values
    for j=1:n
        firstgen(j,:)=binornd(One,.5); %chose binomial because this gives us a 1 or 0 each with probability of .5.  Provides us with a randomly generated string of ones.
    end
    ...
    priceplus=(y-(mktnum.^d)+e-(d.*mktnum+ones(n,1))).^b;%price calculated for oppt cost calculations
...
for g=2:t
 ...
 %%Fitness value
  ...
  for i=1:n
  for j=1:N
      mutate_g(i,j)=binornd(1, probmut); %generates an nXN matrix of 1's and 0's. We use this to determine with positions in each generation mutate, or randomly switch with small probability.   If a one exists in position (i,j) then we switch newgen_g(i,j) from one to a zero or vice versa.
  end  %use binornd because it selects a 1 with probability probmut (probability a mutation will occur) and it selects 0 with probability 1-probmut
  end
